...
void executeNative(struct Context *context){ //Temporal
    ...
    MethodAreaData *method_area = (MethodAreaData*)&heap[current_frame_data->method_area_pointer];
...
void ldc(Frame *current_frame_data, struct Context *context, uint8_t *opcode){
    ...
    MethodAreaData *frame_method_area_data = (MethodAreaData*)&heap[current_frame_data->method_area_pointer];
    ConstantPoolEntry* frame_constant_pool = (ConstantPoolEntry*)&heap[frame_method_area_data->constant_pool_ptr];
...
void areturn(Frame *current_frame_data, struct Context *context, uint8_t *opcode){
    method_info *method = (method_info*)&heap[current_frame_data->method_ptr];
    MethodAreaData *ma_data = (MethodAreaData*)&heap[current_frame_data->method_area_pointer];
...
void getstatic(Frame *current_frame_data, struct Context *context, uint8_t *opcode){
    ...
    MethodAreaData *ma_data = (MethodAreaData*)&heap[context->jvm_stack[context->curr_frame].method_area_pointer];
    ...
    MethodAreaData *oMaData = (MethodAreaData*)&heap[oMaPtr];
...
void invokevirtual(Frame *current_frame_data, struct Context *context, uint8_t *opcode){
    ...
    MethodAreaData *methodArea = (MethodAreaData*)&heap[current_frame_data->method_area_pointer];
    ...
    int oMaPtr = get(method_area_hashmap, class_name);
    MethodAreaData *oMethodArea = (MethodAreaData*)&heap[oMaPtr];
...
void invokestatic(Frame *current_frame_data, struct Context *context, uint8_t *opcode){
    ...
    MethodAreaData *method_area_data = (MethodAreaData*)&heap[current_frame_data->method_area_pointer];
    ConstantPoolEntry *constantPool = (ConstantPoolEntry*)&heap[method_area_data->constant_pool_ptr];

    method_info *methods = (method_info*)&heap[method_area_data->methods_ptr]; //Esto da información del método pero no el método, gracias a esto se puede buscar pero esto no debe usarse porque puede que el método se encuentre en otra clase, es decir, otro method area
    ...
    char *fMethodName = (char*)&heap[constantPool[methods[i].name_index].info.CONSTANT_utf8.text_ptr];
        if (!strcmp(methodName, fMethodName)){
            initFrame(get(method_area_hashmap, class_name), (method_area_data->methods_ptr + (sizeof(method_info) * i)), context);
        }
    }
}
